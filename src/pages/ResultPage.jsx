  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await fetch(`https://satenders-mac-mini.tail443769.ts.net/api/search?query=${query}&type=movie`);
        
        const resSem = await fetch(`https://satenders-mac-mini.tail443769.ts.net/api/searchBert?query=${query}&type=movie`);
  ...
  return (
    <div className="min-h-screen p-4 bg-gray-50">
      ...
      {loading ? (
        ...
        {/* books  */}
        <h2 className="text-2xl font-bold mb-4 mt-6"> Books (Non-Semantic) </h2>
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {books.map((item) => (
            <div
              key={item?.id}
